 ...
 public function iWaitSeconds($arg1) {
  ...
  protected function getMailAddress($user) {
    ...
    if(empty($this->mailAddresses[$user])) {
      ...
      //$this->mailAddresses[$user] = $this->email['address'];
    }
...
//    throw new \Exception('Email "' . $title . '" does not have any links.');
  ...
  /**
   * @Given /^I select "([^"]*)" from ranking table countries filter$/
   */
  public function iSelectFromRankingTableCountriesFilter($option) {
    $option = $this->fixStepArgument($option);
    $select = 'edit-field-country-tid';
    $field = $this->getSession()->getPage()->findField($select, true);
    ...
    $javascript = "jQuery('#$select').val($val);
  ...
  /**
   * @Then /^I reorder "([^"]*)" with class "([^"]*)" to level "([^"]*)"$/
   */
  public function IreOrderVariant($target, $class, $level) {
    $session = $this->getSession();
    $containerDiv = $session->getPage()->findAll('css', $class);
    $end = end($containerDiv);
    $id = $end->find('css', '.tabledrag-hide select')->getAttribute('id');
    $javascript = "jQuery('#$id').val('$level');
                jQuery('#$id').trigger('chosen:updated');
                jQuery('#$id').trigger('change');";
  ...
  /**
   * @When /^I click the admin cog "([^"]*)" link "([^"]*)"$/
   */
  public function clickAdminCog($class, $target) {
    $session = $this->getSession();
    $containerDiv = $session->getPage()->find('css', $class);

    if($containerDiv->isVisible()){
      $containerDiv->click();
      foreach ($session->getPage()->findAll('css',  $class . ' ul') as $list) {
        $listItem = $list->findAll('css', 'li');
        foreach($listItem as $item){
          if($target == $item->getText()) {
            $item->click();
          }
        }
      }
...
JS;
  ...
  /**
   * @Given /^I clear the cache cid "([^"]*)" in bin "([^"]*)" with wildcard "([^"]*)"$/
   */
  public function iClearTheCacheCidInBinWithWildcard($cid = NULL, $bin = NULL, $wildcard = FALSE) {
    try {
      $drush = $this->getDriver();
      $run = "cache_clear_all('{$cid}', '{$bin}', '{$wildcard}');";
      $x = $drush->drush('ev', array('"' . $run . '"'));
      print $x;
    }
    catch (Exception $e) {
      throw new \Exception('PHP evaluation failed. ' . $e->getMessage());
  ...
  public function iGoToTheNodeWithTheTitle($type, $title) {
   ...
   /**
   * @When /^I fill in all required fields on "([^"]*)" form with title "([^"]*)"$/
   */
  public function iFillInRequiredFields($form, $title) {

    $test_text = 'Sample text';

    $element = $this->getSession()->getPage();
    $nodeform = $element->find('css', 'form.node-form');

    $nodetitle = $nodeform->find('css', 'input#edit-title.required');
    $nodetitle->setValue($title);

    $textareas = $nodeform->findAll('css', '.pane-form textarea.required');
    foreach ($textareas as $ta) {
      if($ta->isVisible()){
        $ta->setValue($test_text);
      }else{
        $editorid = $ta->getAttribute('id');
        $instance = $this->getWysiwygInstance($editorid);
        $this->getSession()->executeScript("$instance.insert(\"$test_text\");");
      }
    } // close textarea loop

    $selects = $nodeform->findAll('css', '.pane-form select.required');
    foreach ($selects as $ss) {
      $options = $ss->findAll('css', 'option');
      foreach($options as $idx => $option){
        $lastval = '';
        if($idx+1 == count($options)){
          $lastval = $option->getValue();
        }
        $idx++;
      }
      if($ss->isVisible()){
    ...
    $inputs = $nodeform->findAll('css', '.pane-form input.form-text.required');
    foreach ($inputs as $in) {
      if($in->isVisible()){
       $in->setValue($test_text);
      }
    } // close inputs loop

    //Now repeat for vertical tab fieldsets
    $formrequired = $element->findAll('css', '.vertical-tabs a span.form-required');
    foreach ($formrequired as $fr) {
      $fr->click();
      $fieldsets = $element->findAll('css', 'fieldset.required-fields');
      foreach($fieldsets as $fs){
        if($fs->isVisible()){
          $textareas = $fs->findAll('css', 'textarea.required');
          foreach ($textareas as $ta) {
            if($ta->isVisible()){
              $ta->setValue($test_text);
            }else{
          ...
          $selects = $fs->findAll('css', 'select.required');
          foreach ($selects as $ss) {
            $options = $ss->findAll('css', 'option');
            foreach($options as $idx => $option){
              $lastval = '';
          ...
          $inputs = $fs->findAll('css', 'input.form-text.required');
          foreach ($inputs as $in) {
            $in->setValue($test_text);
          } // close inputs loop
        } // close isVisible
      } // clsoe fieldset lop
    } // close vertical-tabs required loop
  }
  ...
  public function test() {
    ...
    $select = 'edit-field-address-und-0-thoroughfare';
    $page = $this->getSession()->getPage();
    $field = $page->findField($select, false);
    $field2 = $page->findField('Address 1 *', false);
...
//    $this->getSession()->executeScript($javascript);
  ...
  public static function prepare(BeforeSuiteScope $scope)
  {
    ...
    // before it runs
    print 'beforesuite';
    //$scope->getSession()->getDriver()->resizeWindow(1440, 900);
  }
